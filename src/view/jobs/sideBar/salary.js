import React, { useState } from 'react';
import Alert from 'react-bootstrap/Alert';
import Button from 'react-bootstrap/Button';
import 'bootstrap/dist/css/bootstrap.min.css';
import 'bootstrap/dist/js/bootstrap.bundle'
import "./es_jobs.css"
function Salary()
{
    return(
        <>
            <div className='d-flex '>
                <div className='d-flex es_div1_salary'>
                <img className='es_img2' src={"data:image/png;base64,"}/>
                <h6 className='es_font5'>S A L A R Y</h6></div>
                <div className='es_div2_salary d-flex'>
                <h6 className='es_font5'>Esraa Mohamed</h6>
                <img  className='es_img3' src={'https://media-exp1.licdn.com/dms/image/D4D03AQF_Wn6Bih6JiQ/profile-displayphoto-shrink_100_100/0/1665756979155?e=1672272000&v=beta&t=RO8WrMEp8IymETLgfjPD41ZC56-Wa71MTyPcd3Q2YVU'} /></div>
            </div>
            <div className='es_div3_salary'>
                <p className='es_font6'>Coming soon!</p>
                <p>LinkedIn Salary is not yet available in your country.</p>
                <button className='es_button3'>Go to LinkedIn
                </button>
            </div>

        </>
    )
} 
export default Salary;